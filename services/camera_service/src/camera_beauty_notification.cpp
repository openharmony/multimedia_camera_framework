...
void CameraBeautyNotification::GetPixelMap()
{
    ...
    CHECK_ERROR_RETURN_LOG(imageSourceProxy == nullptr, "CreateImageSourceProxy failed");
    auto ret = imageSourceProxy->CreateImageSource(data.get(), len, opts, errorCode);
    CHECK_ERROR_RETURN_LOG(ret != 0, "CreateImageSource failed");
    ...
    MEDIA_INFO_LOG("CreateImageSource errorCode: %{public}d", static_cast<int32_t>(errorCode));
    CHECK_ERROR_RETURN_LOG(pixelMap == nullptr, "Create icon pixel map failed,pixelMap is nullptr");
