...
void CameraReportUtils::RecordDirectorModeOpen(const std::string& bundleName, int32_t pid, int32_t uid)
{
    MEDIA_DEBUG_LOG("CameraReportUtils::RecordDirectorModeOpen bundleName:%{public}s pid:%{public}d uid:%{public}d",
        bundleName.c_str(), pid, uid);

    std::lock_guard<std::mutex> lock(directorModeRecordsMutex_);
    if (hasActiveDirectorMode_) {
        MEDIA_DEBUG_LOG("DirectorMode already active, ignoring duplicate open");
        return;
    }
    DirectorModeRecord record;
    record.openTime = DeferredProcessing::SteadyClock::GetTimestampMilli();
    record.closeTime = 0;
    record.duration = 0;
    ...
    activeDirectorModeRecord_ = record;
    hasActiveDirectorMode_ = true;
    MEDIA_DEBUG_LOG("DirectorMode opened at %{public}" PRIu64 "", record.openTime);
}

void CameraReportUtils::RecordDirectorModeClose()
{
    MEDIA_DEBUG_LOG("CameraReportUtils::RecordDirectorModeClose");
    std::lock_guard<std::mutex> lock(directorModeRecordsMutex_);
    if (!hasActiveDirectorMode_) {
        MEDIA_DEBUG_LOG("DirectorMode not active, ignoring close");
        return;
    }
    activeDirectorModeRecord_.closeTime = DeferredProcessing::SteadyClock::GetTimestampMilli();
    activeDirectorModeRecord_.duration =
        activeDirectorModeRecord_.closeTime - activeDirectorModeRecord_.openTime;
    activeDirectorModeRecord_.isCompleted = true;

    std::lock_guard<std::mutex> lock2(directorModeCompletedRecordsMutex_);
    directorModeCompletedRecords_.push_back(activeDirectorModeRecord_);
    MEDIA_DEBUG_LOG("DirectorMode closed, duration: %{public}" PRIu64 " ms", activeDirectorModeRecord_.duration);

    hasActiveDirectorMode_ = false;
}

void CameraReportUtils::CleanupExpiredDirectorModeRecords()
{
    MEDIA_DEBUG_LOG("CameraReportUtils::CleanupExpiredDirectorModeRecords");
    uint64_t currentTime = DeferredProcessing::SteadyClock::GetTimestampMilli();
    uint64_t expireTime = 24 * 60 * 60 * 1000;

    {
        std::lock_guard<std::mutex> lock(directorModeRecordsMutex_);
        if (hasActiveDirectorMode_ &&
            (currentTime - activeDirectorModeRecord_.openTime) > expireTime) {
            MEDIA_WARNING_LOG("Remove expired unclosed DirectorMode record for bundleName:%{public}s",
                activeDirectorModeRecord_.bundleName.c_str());
            hasActiveDirectorMode_ = false;
        }
        std::lock_guard<std::mutex> lock2(directorModeCompletedRecordsMutex_);
        directorModeCompletedRecords_.remove_if(
            [currentTime, expireTime](const DirectorModeRecord& record) {
                return (currentTime - record.closeTime) > expireTime;
            });
    }
}

void CameraReportUtils::CalculateAndReportDirectorModeStats()
{
    MEDIA_DEBUG_LOG("CameraReportUtils::CalculateAndReportDirectorModeStats");
    std::lock_guard<std::mutex> lock(directorModeCompletedRecordsMutex_);
    if (directorModeCompletedRecords_.empty()) {
        MEDIA_DEBUG_LOG("No completed DirectorMode records to report");
        return;
    }

    std::unordered_map<std::string, DirectorModeStats> appStatsMap;
    for (const auto& record : directorModeCompletedRecords_) {
        if (record.duration > 0) {
            const std::string& bundleName = record.bundleName;
            if (appStatsMap.find(bundleName) == appStatsMap.end()) {
                DirectorModeStats stats;
                stats.minDuration = UINT64_MAX;
                stats.maxDuration = 0;
                stats.totalDuration = 0;
                stats.count = 0;
                appStatsMap[bundleName] = stats;
            }
            DirectorModeStats& stats = appStatsMap[bundleName];
            stats.minDuration = std::min(stats.minDuration, record.duration);
            stats.maxDuration = std::max(stats.maxDuration, record.duration);
            stats.totalDuration += record.duration;
            stats.count++;
        }
    }

    int32_t type = 2;  // TYPE=2 for DirectorMode (TYPE=1 is LiveControlCenter)
    for (const auto& [bundleName, stats] : appStatsMap) {
        uint64_t avgDuration = 0;
        if (stats.count != 0) {
            avgDuration = stats.totalDuration / stats.count;
        }
        MEDIA_DEBUG_LOG("Report DirectorMode stats for app:%{public}s - min:%{public}" PRIu64 " ms, "
                       "max:%{public}" PRIu64 " ms, avg:%{public}" PRIu64 " ms, count:%{public}u",
                       bundleName.c_str(), stats.minDuration, stats.maxDuration, avgDuration, stats.count);
        ...
        HiSysEventWrite(
            ...
            "MODE_USE_COUNT", stats.count,
            "MODE_MIN_USE", stats.minDuration,
            "MODE_MAX_USE", stats.maxDuration,
            "MODE_AVG_USE", avgDuration);
    }
    directorModeCompletedRecords_.clear();
}
} // namespace CameraStandard
} // namespace OHOS
