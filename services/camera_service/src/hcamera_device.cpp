...
static std::mutex g_cameraDeviceInstanceMutex;
static HCameraDevice* g_cameraDeviceInstance = nullptr;
      ...
      deviceMuteMode_(false), isHasOpenSecure(false)
{
    ...
    MEDIA_INFO_LOG("HCameraDevice::HCameraDevice Contructor :%{public}s", clientName_.c_str());
    {
        std::lock_guard<std::mutex> lock(g_cameraDeviceInstanceMutex);
        g_cameraDeviceInstance = this;
    }
}
...
void HCameraDevice::DropDetectionDataCallbackImpl(const OHOS::Rosen::MotionSensorEvent &motionData)
{
        ...
        motionData.type, motionData.status);
    {
        ...
        g_cameraDeviceInstance->cameraHostManager_->NotifyDeviceStateChangeInfo(
