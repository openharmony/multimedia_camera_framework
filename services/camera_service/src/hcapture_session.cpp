    ...
    sptr<CameraServerPhotoProxy> cameraPhotoProxy, std::string &uri)
{
    ...
    if (isBursting) {
        ...
        MEDIA_DEBUG_LOG("CreateMediaLibrary GetPhotoAssetInstance %{public}d E", captureId);
        photoAssetProxy = captureStream->GetPhotoAssetInstance(captureId);
        MEDIA_DEBUG_LOG("CreateMediaLibrary GetPhotoAssetInstance %{public}d X", captureId);
    }
