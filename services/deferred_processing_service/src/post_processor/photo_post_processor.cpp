...
#include "v1_7/iimage_process_service.h"
...
class PhotoPostProcessor::PhotoProcessListener : public OHOS::HDI::Camera::V1_7::IImageProcessCallback {
public:
    ...
    int32_t OnProcessDone_V1_7(
        const std::string& imageId, const std::vector<OHOS::HDI::Camera::V1_7::ImageBufferInfo_V1_7>& buffers) override
    {
        DP_INFO_LOG("OnProcessDone_V1_7 DPS_PHOTO: imageId: %{public}s", imageId.c_str());
        auto processResult = processResult_.lock();
        DP_CHECK_ERROR_RETURN_RET_LOG(processResult == nullptr, DP_OK, "PhotoProcessResult is nullptr.");

        auto ret = processResult->ProcessPictureInfoV1_7(imageId, buffers);
        if (ret != DP_OK) {
            HILOG_COMM_ERROR("Process yuv1_7 done failed imageId: %{public}s.", imageId.c_str());
            processResult->OnError(imageId, DPS_ERROR_IMAGE_PROC_FAILED);
        }
        return DP_OK;
    }
...
void PhotoPostProcessor::OnServiceChange(const HDI::ServiceManager::V1_0::ServiceStatus& status)
{
    ...
    sptr<OHOS::HDI::Camera::V1_7::IImageProcessService> proxyV1_7;
    // LCOV_EXCL_START
    if (versionId >= HDI_VERSION_ID_1_7) {
        proxyV1_7 = OHOS::HDI::Camera::V1_7::IImageProcessService::CastFrom(proxyV1_2);
    } else if (versionId >= HDI_VERSION_ID_1_6) {
    ...
    if (proxyV1_7 != nullptr) {
        DP_INFO_LOG("CreateImageProcessSession_V1_7 version=%{public}d_%{public}d", majorVer, minorVer);
        proxyV1_7->CreateImageProcessSession_V1_7(userId_, processListener_, session);
    } else if (proxyV1_6 != nullptr) {
