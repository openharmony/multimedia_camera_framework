...
int32_t CaptureSessionForSys::SetImagingMode(ImagingMode imagingMode)
{
    ...
    CHECK_PRINT_DLOG(errCode != CameraErrorCode::SUCCESS,
        "CaptureSessionForSys::SetImagingMode Failed to set imaging mode");
    return CameraErrorCode::SUCCESS;
}
...
int32_t CaptureSessionForSys::GetSupportedImagingMode(std::vector<ImagingMode>& imagingMode)
{
    ...
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(!inputDevice || !inputDevice->GetCameraDeviceInfo(), CameraErrorCode::SUCCESS,
        "CaptureSessionForSys::GetSupportedImagingMode camera device is null");
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(
    ...
    CHECK_RETURN_RET_ELOG(ret != CAM_META_SUCCESS, CameraErrorCode::SUCCESS,
        "CaptureSessionForSys::GetSupportedImagingMode Failed with return code %{public}d", ret);
