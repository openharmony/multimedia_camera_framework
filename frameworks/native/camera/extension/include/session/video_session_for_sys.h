...
#ifndef OHOS_CAMERA_VIDEO_SESSION_FOR_SYS_H
#define OHOS_CAMERA_VIDEO_SESSION_FOR_SYS_H
...
class VideoSessionForSys : public CaptureSessionForSys {
public:
    class VideoSessionMetadataResultProcessor : public MetadataResultProcessor {
    public:
        explicit VideoSessionMetadataResultProcessor(wptr<VideoSessionForSys> session) : session_(session) {}
        void ProcessCallbacks(const uint64_t timestamp,
            const std::shared_ptr<OHOS::Camera::CameraMetadata>& result) override;

    private:
        wptr<VideoSessionForSys> session_;
    };

    explicit VideoSessionForSys(sptr<ICaptureSession> &videoSessionForSys): CaptureSessionForSys(videoSessionForSys)
    {
...
#endif // OHOS_CAMERA_VIDEO_SESSION_FOR_SYS_H