...
/*
 * Feature: HCaptureSession
 * Function: Test multiple Start and Stop operations
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test multiple Start and Stop operations in sequence
 */
HWTEST_F(HCaptureSessionUnitTest, hcapture_session_unit_test_077, TestSize.Level1)
{
    std::vector<string> cameraIds;
    cameraService_->GetCameraIds(cameraIds);
    ASSERT_NE(cameraIds.size(), 0);
...
/*
 * Feature: HCaptureSession
 * Function: Test SetCallback and UnSetCallback
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test SetCallback with valid callback and UnSetCallback
 */
HWTEST_F(HCaptureSessionUnitTest, hcapture_session_unit_test_078, TestSize.Level1)
{
    std::vector<string> cameraIds;
    cameraService_->GetCameraIds(cameraIds);
    ASSERT_NE(cameraIds.size(), 0);
    ...
    sptr<MockHCaptureSessionCallbackStub> callback = new MockHCaptureSessionCallbackStub();
    EXPECT_EQ(session->SetCallback(callback), CAMERA_OK);
    EXPECT_EQ(session->UnSetCallback(), CAMERA_OK);
    EXPECT_EQ(session->SetCallback(callback), CAMERA_OK);
...
/*
 * Feature: HCaptureSession
 * Function: Test SetPressureCallback and UnSetPressureCallback
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test SetPressureCallback with valid callback and UnSetPressureCallback
 */
HWTEST_F(HCaptureSessionUnitTest, hcapture_session_unit_test_079, TestSize.Level1)
{
    std::vector<string> cameraIds;
    cameraService_->GetCameraIds(cameraIds);
    ASSERT_NE(cameraIds.size(), 0);
    ...
    sptr<MockHPressureStatusCallbackStub> pressureCallback = new MockHPressureStatusCallbackStub();
    EXPECT_EQ(session->SetPressureCallback(pressureCallback), CAMERA_OK);
    EXPECT_EQ(session->UnSetPressureCallback(), CAMERA_OK);
    EXPECT_EQ(session->SetPressureCallback(pressureCallback), CAMERA_OK);
...
/*
 * Feature: HCaptureSession
 * Function: Test AddOutput with MOVIE_FILE RepeatStreamType
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test AddOutput and operations with MOVIE_FILE RepeatStreamType
 */
HWTEST_F(HCaptureSessionUnitTest, hcapture_session_unit_test_082, TestSize.Level1)
{
    std::vector<string> cameraIds;
    cameraService_->GetCameraIds(cameraIds);
    ASSERT_NE(cameraIds.size(), 0);
    ...
    sptr<HStreamRepeat> streamRepeat = new (std::nothrow) HStreamRepeat(producer, DEFAULT_FORMAT,
        DEFAULT_WIDTH, DEFAULT_HEIGHT, RepeatStreamType::PREVIEW);
    ASSERT_NE(streamRepeat, nullptr);
    sptr<HStreamRepeat> movieFileStream = new (std::nothrow) HStreamRepeat(producer, DEFAULT_FORMAT,
        DEFAULT_WIDTH, DEFAULT_HEIGHT, RepeatStreamType::MOVIE_FILE);
    ASSERT_NE(movieFileStream, nullptr);

    EXPECT_EQ(session->AddOutput(StreamType::REPEAT, streamRepeat), CAMERA_OK);
    EXPECT_EQ(session->AddOutput(StreamType::REPEAT, movieFileStream), CAMERA_OK);
    EXPECT_EQ(session->CommitConfig(), CAMERA_OK);
...
/*
 * Feature: HCaptureSession
 * Function: Test AddOutput with SKETCH RepeatStreamType
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test AddOutput and operations with SKETCH RepeatStreamType
 */
HWTEST_F(HCaptureSessionUnitTest, hcapture_session_unit_test_083, TestSize.Level1)
{
    std::vector<string> cameraIds;
    cameraService_->GetCameraIds(cameraIds);
    ASSERT_NE(cameraIds.size(), 0);
    ...
    sptr<HStreamRepeat> streamRepeat = new (std::nothrow) HStreamRepeat(producer, DEFAULT_FORMAT,
        DEFAULT_WIDTH, DEFAULT_HEIGHT, RepeatStreamType::PREVIEW);
    ASSERT_NE(streamRepeat, nullptr);
    sptr<HStreamRepeat> sketchStream = new (std::nothrow) HStreamRepeat(producer, DEFAULT_FORMAT,
        DEFAULT_WIDTH, DEFAULT_HEIGHT, RepeatStreamType::SKETCH);
    ASSERT_NE(sketchStream, nullptr);

    EXPECT_EQ(session->AddOutput(StreamType::REPEAT, streamRepeat), CAMERA_OK);
    EXPECT_EQ(session->AddOutput(StreamType::REPEAT, sketchStream), CAMERA_OK);
    EXPECT_EQ(session->CommitConfig(), CAMERA_OK);
    EXPECT_EQ(session->Start(), CAMERA_OK);
    EXPECT_EQ(session->Stop(), CAMERA_OK);
...
/*
 * Feature: HCaptureSession
 * Function: Test CommitConfig without AddInput
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test CommitConfig when no input device is added
 */
HWTEST_F(HCaptureSessionUnitTest, hcapture_session_unit_test_084, TestSize.Level1)
{
    std::vector<string> cameraIds;
    cameraService_->GetCameraIds(cameraIds);
    ASSERT_NE(cameraIds.size(), 0);
...
/*
 * Feature: HCaptureSession
 * Function: Test RemoveOutput after Start
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test RemoveOutput operation after session is started
 */
HWTEST_F(HCaptureSessionUnitTest, hcapture_session_unit_test_085, TestSize.Level1)
{
    std::vector<string> cameraIds;
    cameraService_->GetCameraIds(cameraIds);
    ASSERT_NE(cameraIds.size(), 0);
    ...
    EXPECT_EQ(session->BeginConfig(), CAMERA_OK);
    EXPECT_EQ(session->RemoveOutput(StreamType::REPEAT, streamRepeat2), CAMERA_OK);
    EXPECT_EQ(session->CommitConfig(), CAMERA_OK);
