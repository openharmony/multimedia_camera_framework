...
HWTEST_F(CaptureSessionUnitTest, capture_session_unit_217, TestSize.Level0)
{
    ...
    int32_t retCode = session->GetSupportedImagingMode(imagingModes);
    EXPECT_EQ(retCode, CameraErrorCode::SUCCESS);
    EXPECT_EQ(session->UnlockForControl(), CameraErrorCode::SUCCESS);
...
HWTEST_F(CaptureSessionUnitTest, capture_session_unit_218, TestSize.Level0)
{
    ...
    session->LockForControl();
    std::shared_ptr<Camera::CameraMetadata> metadata = session->GetMetadata();
    ASSERT_NE(metadata, nullptr);
    ...
    session->changedMetadata_->addEntry(OHOS_ABILITY_IMAGING_MODES, testTypes.data(), testTypes.size());
    EXPECT_EQ(session->UnlockForControl(), CameraErrorCode::SUCCESS);
    ...
    bool isSupported;
    EXPECT_EQ(session->IsImagingModeSupported(ImagingMode::IMAGING_MODE_RGB, isSupported), CameraErrorCode::SUCCESS);
    EXPECT_TRUE(isSupported);

    ImagingMode imagingMode = static_cast<ImagingMode>(5);
    EXPECT_FALSE(session->IsImagingModeSupported(imagingMode));
...
HWTEST_F(CaptureSessionUnitTest, camera_framework_unittest_219, TestSize.Level0)
{
    ...
    ImagingMode imagingMode = static_cast<ImagingMode>(5);
    ASSERT_NE(session->SetImagingMode(imagingMode), 0);
...
HWTEST_F(CaptureSessionUnitTest, camera_framework_unittest_0220, TestSize.Level0)
{
    ...
    bool isSupported;
    EXPECT_EQ(session->IsImagingModeSupported(imagingMode, isSupported), CameraErrorCode::SUCCESS);
    if (isSupported) {
        EXPECT_TRUE(session->IsImagingModeSupported(imagingMode));
        EXPECT_EQ(session->SetImagingMode(imagingMode), CameraErrorCode::SUCCESS);
    } else {
        EXPECT_EQ(session->SetImagingMode(imagingMode), CameraErrorCode::OPERATION_NOT_ALLOWED);
    }
...
HWTEST_F(CaptureSessionUnitTest, capture_session_unit_221, TestSize.Level0)
{
    ...
    session->changedMetadata_->addEntry(OHOS_CONTROL_IMAGING_MODE, &testType, 1);
    EXPECT_EQ(session->UnlockForControl(), CameraErrorCode::SUCCESS);

    ImagingMode imagingMode;
    int32_t retCode = session->GetActiveImagingMode(imagingMode);
    EXPECT_EQ(retCode, CameraErrorCode::SUCCESS);
