    ...
    const vector<vector<int32_t>>& b)
{
    ...
    for (const auto& vecA : a) {
        for (const auto& vecB : b) {
            if (AreVectorEqual(vecA, vecB)) {
...
sptr<CaptureOutput> CameraVideoSessionUnitTest::CreatePreviewOutput(Profile previewProfile)
{
...
sptr<CaptureOutput> CameraVideoSessionUnitTest::CreateVideoOutput(VideoProfile videoProfile)
{
    sptr<Surface> surface = Surface::CreateSurfaceAsConsumer();
    if (surface == nullptr) {
        return nullptr;
...
void CameraVideoSessionUnitTest::UpdataCameraOutputCapability(int32_t modeName, std::vector<sptr<CameraDevice>> cameras)
{
    ...
    auto outputCapability = cameraManager_->GetSupportedOutputCapability(cameras[0], modeName);
    ASSERT_NE(outputCapability, nullptr);

    previewProfiles_ = outputCapability->GetPreviewProfiles();
    ASSERT_FALSE(previewProfiles_.empty());
    ...
    ASSERT_FALSE(videoProfiles_.empty());
}
...
/*
 * Feature: Framework
 * Function: Test setFrameRate method
 * SubFunction: NA
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test whether the setFrameRate method of previewOutput is
 *                  successful after setting the setFrameRate method of videoOutput.
 */
HWTEST_F(CameraVideoSessionUnitTest, video_session_unittest_011, TestSize.Level0)
{
    std::vector<sptr<CameraDevice>> cameras = cameraManager_->GetSupportedCameras();
    sptr<CaptureInput> input = cameraManager_->CreateCameraInput(cameras[0]);
    ASSERT_NE(input, nullptr);
    ...
    sptr<CaptureOutput> video = CreateVideoOutput(videoProfile);
    ASSERT_NE(video, nullptr);
    sptr<VideoOutput> videoOutput = (sptr<VideoOutput>&) video;
    ...
    vector<vector<int32_t>> previewSupportedFrameRateList = previewOutput->GetSupportedFrameRates();
 ...
 * FunctionPoints: NA
 * EnvConditions: NA
 * CaseDescription: Test whether the setFrameRate method of videoOutput is
 *                  successful after setting the setFrameRate method of previewOutput.
 */
