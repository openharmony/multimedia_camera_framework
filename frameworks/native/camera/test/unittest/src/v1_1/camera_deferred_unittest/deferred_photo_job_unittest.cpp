...
const int g_defaultPhotoJobType = 0;
const int g_testPhotoJobType = 1;
const std::string g_testImageId = "testImageId";
...
void DeferredPhotoJobUnitTest::TestRegisterJobListener(std::shared_ptr<PhotoJobRepository> photoJR, const int32_t userId)
{    
...
HWTEST_F(DeferredPhotoJobUnitTest, deferred_photo_job_unittest_001, TestSize.Level0)
{
    ...
    std::shared_ptr<DeferredPhotoJob> derferredPhotoJob =
        std::make_shared<DeferredPhotoJob>(g_testImageId, testDiscardable, metadata);

    ASSERT_NE(derferredPhotoJob, nullptr);
    EXPECT_EQ(derferredPhotoJob->GetDiscardable(), testDiscardable);
    EXPECT_EQ(derferredPhotoJob->GetImageId(), g_testImageId);
    ...
    EXPECT_EQ(derferredPhotoJob->GetPhotoJobType(), g_defaultPhotoJobType);
    derferredPhotoJob->SetPhotoJobType(g_testPhotoJobType);
    EXPECT_EQ(derferredPhotoJob->GetPhotoJobType(), g_testPhotoJobType);
...
HWTEST_F(DeferredPhotoJobUnitTest, deferred_photo_job_unittest_002, TestSize.Level0)
{
    ...
    EXPECT_EQ(deferredPhotoWork->GetDeferredPhotoJob()->GetPhotoJobType(), g_testPhotoJobType);
}
...
HWTEST_F(DeferredPhotoJobUnitTest, deferred_photo_job_unittest_003, TestSize.Level0)
{
    ...
    std::shared_ptr<PhotoJobRepository> photoJR = std::make_shared<PhotoJobRepository>(g_userId);
    ...
    EXPECT_FALSE(photoJR->IsOfflineJob(g_testImageId));
    EXPECT_FALSE(photoJR->HasUnCompletedBackgroundJob());

    TestRegisterJobListener(photoJR, g_userId);
    ASSERT_NE(photoJR, nullptr);
...
HWTEST_F(DeferredPhotoJobUnitTest, deferred_photo_job_unittest_004, TestSize.Level0)
{
    ...
    EXPECT_EQ(photoJR->GetJobUnLocked(g_testImageId), nullptr);
    photoJR->RemoveDeferredJob(g_testImageId, true);
    photoJR->RequestJob(g_testImageId);
    photoJR->CancelJob(g_testImageId);
    photoJR->RestoreJob(g_testImageId);
    photoJR->SetJobPending(g_testImageId);
    photoJR->SetJobRunning(g_testImageId);
    photoJR->SetJobCompleted(g_testImageId);
    photoJR->SetJobFailed(g_testImageId);
    EXPECT_EQ(photoJR->GetJobStatus(g_testImageId), PhotoJobStatus::NONE);
    EXPECT_EQ(photoJR->GetJobRunningPriority(g_testImageId), PhotoJobPriority::NONE);
    EXPECT_EQ(photoJR->GetJobPriority(g_testImageId), PhotoJobPriority::NONE);
    EXPECT_EQ(photoJR->GetJobUnLocked(g_testImageId), nullptr);
}
