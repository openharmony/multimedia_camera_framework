...
HWTEST_F(CameraPhotoOutputUnit, photo_output_unittest_019, TestSize.Level0)
{
    ...
    int ret_3 = OHOS::Camera::FindCameraMetadataItem(settings->captureMetadataSetting_->get(), OHOS_CONTROL_BURST_CAPTURE, &item);
    ASSERT_TRUE(ret_3 == CAM_META_SUCCESS && item.count > 0);
    EXPECT_EQ(item.data.u8[0], 1);
    settings->SetBurstCaptureState(2);
    ret_3 = OHOS::Camera::FindCameraMetadataItem(settings->captureMetadataSetting_->get(), OHOS_CONTROL_BURST_CAPTURE, &item);
    ASSERT_TRUE(ret_3 == CAM_META_SUCCESS && item.count > 0);
...
HWTEST_F(CameraPhotoOutputUnit, photo_output_unittest_020, TestSize.Level0)
{
    ...
    int32_t captureId = 1;
    auto callback = new (std::nothrow) HStreamCaptureCallbackImpl(phtOutput);
