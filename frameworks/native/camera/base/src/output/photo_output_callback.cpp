...
int32_t HStreamCaptureThumbnailCallbackImpl::OnThumbnailAvailable(sptr<SurfaceBuffer> surfaceBuffer, int64_t timestamp)
{
    ...
    surfaceBuffer->GetExtraData()->ExtraGet(OHOS::Camera::captureId, info.captureID);
    surfaceBuffer->GetExtraData()->ExtraGet(OHOS::Camera::expoTime, info.expoTime);
    surfaceBuffer->GetExtraData()->ExtraGet(OHOS::Camera::expoIso, info.expoIso);
    surfaceBuffer->GetExtraData()->ExtraGet(OHOS::Camera::expoFNumber, info.expoFNumber);
