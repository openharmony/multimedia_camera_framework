...
int32_t ScanSession::AddOutput(sptr<CaptureOutput> &output)
{
    ...
    CHECK_RETURN_RET_ELOG((outputCapability == nullptr || outputCapability->GetPreviewProfiles().size() == 0 ||
        ...
        CameraErrorCode::SESSION_NOT_CONFIG, "ScanSession::AddOutput can not add current type of output");
...
bool ScanSession::CanAddOutput(sptr<CaptureOutput> &output)
{
    ...
    CHECK_RETURN_RET_ELOG(
        !IsSessionConfiged() || output == nullptr, false, "ScanSession::CanAddOutput operation is Not allowed!");
...
{
    CHECK_RETURN_RET_ELOG(
        !IsSessionCommited(), false, "ScanSession::IsBrightnessStatusSupported Session is not Commited");
    // LCOV_EXCL_START
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(!inputDevice, false, "ScanSession::IsBrightnessStatusSupported camera device is null");
    auto device = inputDevice->GetCameraDeviceInfo();
    CHECK_RETURN_RET_ELOG(!device, false, "ScanSession::IsBrightnessStatusSupported camera deviceInfo is null");
...
{
    // LCOV_EXCL_START
    CHECK_RETURN_ELOG(!IsSessionCommited(), "ScanSession::RegisterBrightnessStatusCallback Session is not Commited");
...
{
    // LCOV_EXCL_START
    CHECK_RETURN_ELOG(!IsSessionCommited(), "ScanSession::UnRegisterBrightnessStatusCallback Session is not Commited");
...
void ScanSession::ProcessBrightnessStatusChange(const std::shared_ptr<OHOS::Camera::CameraMetadata>& result)
{
    ...
    CHECK_RETURN_ELOG(
        ret != CAM_META_SUCCESS, "ScanSession::ProcessBrightnessStatusChange get brightness status failed");
    bool state = true;
    ...
    CHECK_RETURN(state == lastBrightnessStatus_);
    callback->OnBrightnessStatusChanged(state);
    lastBrightnessStatus_ = state;
    // LCOV_EXCL_STOP
}
    ...
    const uint64_t timestamp, const std::shared_ptr<OHOS::Camera::CameraMetadata>& result)
{
    ...
    CHECK_RETURN_ELOG(
        session == nullptr, "ScanSession::ScanSessionMetadataResultProcessor ProcessCallbacks but session is null");
