...
int32_t FoldCallback::OnFoldStatusChanged(const FoldStatus status)
{
    ...
    CHECK_RETURN_RET_ELOG(autoDeviceSwitchCallback == nullptr, CAMERA_OPERATION_NOT_ALLOWED,
...
int32_t CaptureSession::CommitConfig()
{
    ...
    std::vector<std::string> streamTypes;
    std::vector<int32_t> formats;
    std::vector<std::string> fpss;
    std::vector<uint32_t> size = {0, 0};
    Size previewSize = {0, 0};
    for (const auto& output : captureOutputSets_) {
        auto item = output.promote();
        std::string type = mapStreamType.find(item->GetStreamType()) == mapStreamType.end() ?
            "UnknownStreamType" : mapStreamType.at(item->GetStreamType());
        MEDIA_DEBUG_LOG("CaptureSession::CommitConfig stream type is %{public}s", type.c_str());
        ...
        if (item->GetOutputType() == CAPTURE_OUTPUT_TYPE_PREVIEW) {
            ...
            vector<int32_t> previewFrameRateRange = previewOutput->GetFrameRateRange();
            fps = Container2String(previewFrameRateRange.begin(), previewFrameRateRange.end());
            auto previewProfile = previewOutput->GetPreviewProfile();
            if(previewProfile != nullptr){
                MEDIA_DEBUG_LOG("CaptureSession::CommitConfig previewProfile is not nullptr!");
                format = static_cast<int32_t>(previewProfile->GetCameraFormat());
                size = {previewProfile->size_.width, previewProfile->size_.height};
                previewSize = previewProfile->size_;
            }
        } else if (item->GetOutputType() == CAPTURE_OUTPUT_TYPE_PHOTO) {
            sptr<PhotoOutput> photoOutput = (sptr<PhotoOutput>&)item;
            auto photoProfile = photoOutput->GetPhotoProfile();
            if(photoProfile != nullptr){
                MEDIA_DEBUG_LOG("CaptureSession::CommitConfig photoProfile is not nullptr!");
                format = static_cast<int32_t>(photoProfile->GetCameraFormat());
                size = {photoProfile->size_.width, photoProfile->size_.height};
            }
        } else if (item->GetOutputType() == CAPTURE_OUTPUT_TYPE_DEPTH_DATA) {
            auto depthProfile = item->GetDepthProfile();
            if(depthProfile != nullptr){
                MEDIA_DEBUG_LOG("CaptureSession::CommitConfig depthProfile is not nullptr!");
                format = static_cast<int32_t>(depthProfile->GetCameraFormat());
                size = {depthProfile->size_.width, depthProfile->size_.height};
            }
        } else if (item->GetOutputType() != CAPTURE_OUTPUT_TYPE_METADATA){
            sptr<VideoOutput> videoOutput = (sptr<VideoOutput>&)item;
            vector<int32_t> videoFrameRateRange = videoOutput->GetFrameRateRange();
            fps = Container2String(videoFrameRateRange.begin(), videoFrameRateRange.end());
            auto videoProfile = videoOutput->GetVideoProfile();
            if(videoProfile != nullptr){
                MEDIA_DEBUG_LOG("CaptureSession::CommitConfig videoProfile is not nullptr!");
                format = static_cast<int32_t>(videoProfile->GetCameraFormat());
                size = {videoProfile->size_.width, videoProfile->size_.height};
            }
        }
        type += ":" + Container2String(size.begin(), size.end());
        MEDIA_DEBUG_LOG("CaptureSession::CommitConfig stream format is %{public}d", format);
        MEDIA_DEBUG_LOG("CaptureSession::CommitConfig stream size is %{public}d, %{public}d", size[0], size[1]);
        formats.emplace_back(format);
        streamTypes.emplace_back(type);
        fpss.emplace_back(fps);
    }
    
    uint count = captureOutputSets_.size();
    std::string streamType = Container2String(streamTypes.begin(), streamTypes.end());
    std::string format = Container2String(formats.begin(), formats.end());
    std::string fps = Container2String(fpss.begin(), fpss.end());
    std::string cameraId = "none";
    auto device = GetInputDevice();
    if(device != nullptr){
    ...
    MEDIA_DEBUG_LOG("CaptureSession::CommitConfig camera id is %{public}s", cameraId.c_str());

    SceneMode mode = GetMode();
    MEDIA_DEBUG_LOG("CaptureSession::CommitConfig mode is %{public}d", mode);

    camera_metadata_item_t item;
    if(GetMetadata() == nullptr) {
        MEDIA_DEBUG_LOG("CaptureSession::CommitConfig get Metadata is nullptr");
        return ServiceToCameraError(errCode);
    }
    int ret = Camera::FindCameraMetadataItem(GetMetadata()->get(), OHOS_CONTROL_NIGHT_SUB_MODE, &item);
    uint nightSubMode = 0;
    if (ret == CAM_META_SUCCESS && item.count > 0) {
        nightSubMode = item.data.u8[0];
        MEDIA_DEBUG_LOG("CaptureSession::CommitConfig night submode is %{public}d", nightSubMode);
    }
    ret = Camera::FindCameraMetadataItem(GetMetadata()->get(), OHOS_ABILITY_LIGHT_PAINTING_TYPE, &item);
    uint lightPaintingType = 0;
    if (ret == CAM_META_SUCCESS && item.count > 0) {
        lightPaintingType = item.data.u8[0];
        MEDIA_DEBUG_LOG("CaptureSession::CommitConfig lightpainting type is %{public}d", lightPaintingType);
    }
    
    auto preconfigProfiles = GetPreconfigProfiles();
    ColorSpace colorSpace = ColorSpace::COLOR_SPACE_UNKNOWN;
    if (preconfigProfiles != nullptr) {
        colorSpace = preconfigProfiles->colorSpace;
    }
    MEDIA_DEBUG_LOG("CaptureSession::CommitConfig colorSpace is %{public}d", colorSpace);

    std::vector<float> zrr = GetZoomRatioRange();
    std::string zoomRatioRange = Container2String(zrr.begin(), zrr.end());
    MEDIA_DEBUG_LOG("CaptureSession::CommitConfig zoom ratio range is %{public}s", zoomRatioRange.c_str());

    VideoStabilizationMode videoStabilizationMode = GetActiveVideoStabilizationMode();
    MEDIA_DEBUG_LOG("CaptureSession::CommitConfig video stabilization mode is %{public}d", videoStabilizationMode);

    POWERMGR_SYSEVENT_CAMERA_CONFIG(streamType, static_cast<int32_t>(previewSize.width),
        static_cast<int32_t>(previewSize.height), cameraId, static_cast<uint>(mode), nightSubMode,
        lightPaintingType, format, fps, static_cast<int32_t>(colorSpace), isColorSpaceSetted_,
        static_cast<uint>(videoStabilizationMode), zoomRatioRange, count, errCode);
...
void CaptureSession::CheckSpecSearch()
{
    ...
    CHECK_RETURN_ELOG(!inputDevice || !(inputDevice->GetCameraDeviceInfo()), "camera device is null");
    camera_metadata_item_t item;
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_ELOG(metadata == nullptr, "CheckSpecSearch camera metadata is null");
...
bool CaptureSession::CheckLightStatus()
{
    camera_metadata_item_t item;
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, false, "CheckLightStatus camera metadata is null");
                                                                     ...
                                                                     bool isForApp)
{
    ...
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, cameraAbilityArray,
...
bool CaptureSession::CanAddInput(sptr<CaptureInput>& input)
{
    ...
    CHECK_RETURN_RET_ELOG(!IsSessionConfiged() || input == nullptr, ret,
        "CaptureSession::AddInput operation Not allowed!");
    auto captureSession = GetCaptureSession();
    CHECK_RETURN_RET_ELOG(captureSession == nullptr, ret,
...
{
    CHECK_RETURN_ELOG(device == nullptr, "CaptureSession::GetMetadataFromService device is nullptr");
    auto cameraId = device->GetID();
    auto serviceProxy = CameraManager::GetInstance()->GetServiceProxy();
    CHECK_RETURN_ELOG(serviceProxy == nullptr, "CaptureSession::GetMetadataFromService serviceProxy is null");
...
int32_t CaptureSession::AddOutput(sptr<CaptureOutput>& output)
{
    ...
    CHECK_RETURN_RET_ELOG(!CanAddOutput(output), ServiceToCameraError(CAMERA_INVALID_ARG),
    ...
    sptr<ICaptureSession>& captureSession)
{
    ...
    if (GetMode() == SceneMode::VIDEO && output->GetOutputType() == CAPTURE_OUTPUT_TYPE_VIDEO) {
        ...
        CHECK_RETURN_RET_ELOG(videoFrameRates.empty(), CameraErrorCode::INVALID_ARGUMENT,
...
bool CaptureSession::CanAddOutput(sptr<CaptureOutput>& output)
{
    ...
    CHECK_RETURN_RET(profilePtr == nullptr, false);
...
int32_t CaptureSession::RemoveInput(sptr<CaptureInput>& input)
{
    ...
    CHECK_RETURN_RET_ELOG(input == nullptr, ServiceToCameraError(CAMERA_INVALID_ARG),
        "CaptureSession::RemoveInput input is null");
    auto device = ((sptr<CameraInput>&)input)->GetCameraDevice();
    CHECK_RETURN_RET_ELOG(device == nullptr, ServiceToCameraError(CAMERA_INVALID_ARG),
...
int32_t CaptureSession::RemoveOutput(sptr<CaptureOutput>& output)
{
    ...
    if (output->GetOutputType() == CAPTURE_OUTPUT_TYPE_METADATA) {
        ...
        CHECK_RETURN_RET_ELOG(!metaOutput, ServiceToCameraError(CAMERA_INVALID_ARG),
...
void CaptureSession::SetPressureCallback(std::shared_ptr<PressureCallback> callback)
{
    CHECK_PRINT_ELOG(callback == nullptr,
        "CaptureSession::SetPressureCallback Unregistering application callback!");
    int32_t errorCode = CAMERA_OK;
    std::lock_guard<std::mutex> lock(sessionCallbackMutex_);
    ...
    if (appPressureCallback_ != nullptr && captureSession != nullptr) {
        ...
        if (captureSession) {
            ...
            if (errorCode != CAMERA_OK) {
                MEDIA_ERR_LOG("CaptureSession::SetPressureCallback: Failed to register callback, errorCode: %{public}d",
                    errorCode);
                pressureStatusCallback_ = nullptr;
                appPressureCallback_ = nullptr;
            }
...
{
    CAMERA_SYNC_TRACE;
    CHECK_RETURN_RET_ELOG(!changedMetadata,
    ...
    CHECK_RETURN_RET_ELOG(baseMetadata == nullptr, CameraErrorCode::SUCCESS,
...
VideoStabilizationMode CaptureSession::GetActiveVideoStabilizationMode()
{
    ...
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(!inputDevice, OFF,
        "CaptureSession::GetActiveVideoStabilizationMode camera device is null");
    auto inputDeviceInfo = inputDevice->GetCameraDeviceInfo();
    CHECK_RETURN_RET_ELOG(!inputDeviceInfo, OFF,
    ...
    CHECK_RETURN_RET_ELOG(metadata == nullptr, OFF,
...
{
    CHECK_RETURN_RET((!CameraSecurity::CheckSystemApp()) && (stabilizationMode == VideoStabilizationMode::HIGH),
        false);
...
int32_t CaptureSession::GetExposureValue(float& exposureValue)
{
    ...
    CHECK_RETURN_RET_ELOG(ret != CAM_META_SUCCESS, 0,
        "CaptureSession::GetExposureValue Failed with return code %{public}d", ret);
...
Point CaptureSession::CoordinateTransform(Point point)
{
    ...
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(!inputDevice, unifyPoint, "CaptureSession::CoordinateTransform cameraInput is nullptr");
    auto inputDeviceInfo = inputDevice->GetCameraDeviceInfo();
    CHECK_RETURN_RET_ELOG(!inputDeviceInfo, unifyPoint,
...
{
    CHECK_PRINT_ELOG(zoomRatioRange.empty(), "zoomRatioRange is empty.");
...
int32_t CaptureSession::PrepareZoom()
{
    ...
    CHECK_PRINT_ELOG(!status, "CaptureSession::PrepareZoom Failed to prepare zoom");
    return CameraErrorCode::SUCCESS;
}
...
int32_t CaptureSession::VerifyAbility(uint32_t ability)
{
    ...
    CHECK_RETURN_RET_ELOG(itrPhoto == photoAbilityId.end() || itrPreview == previewAbilityId.end(),
...
std::vector<FilterType> CaptureSession::GetSupportedFilters()
{
    ...
    // LCOV_EXCL_START
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, supportedFilters,
...
FilterType CaptureSession::GetFilter()
{
    ...
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, FilterType::NONE,
    ...
    CHECK_RETURN_RET(itr != metaFilterTypeMap_.end(), itr->second);
...
std::vector<BeautyType> CaptureSession::GetSupportedBeautyTypes()
{
    ...
    CHECK_RETURN_RET_ELOG(ret != CAMERA_OK, supportedBeautyTypes,
        "CaptureSession::GetSupportedBeautyTypes abilityId is NULL");
    // LCOV_EXCL_START
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, supportedBeautyTypes,
...
std::vector<int32_t> CaptureSession::GetSupportedBeautyRange(BeautyType beautyType)
{
    ...
    CHECK_RETURN_RET_ELOG(std::find(supportedBeautyTypes.begin(), supportedBeautyTypes.end(), beautyType) ==
        supportedBeautyTypes.end(), supportedBeautyRange, "CaptureSession::GetSupportedBeautyRange beautyType is NULL");
    // LCOV_EXCL_START
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, supportedBeautyRange,
...
CameraPosition CaptureSession::GetUsedAsPosition()
{
    ...
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(inputDevice == nullptr, usedAsCameraPosition,
...
void CaptureSession::SetBeauty(BeautyType beautyType, int value)
{
    ...
    if (CAMERA_POSITION_UNSPECIFIED == usedAsCameraPosition) {
        ...
        CHECK_RETURN_ELOG(itr == supportedBeautyTypes.end(),
            "CaptureSession::GetSupportedBeautyTypes abilityId is NULL");
    }
    // LCOV_EXCL_START
...
float CaptureSession::GetMinimumFocusDistance() __attribute__((no_sanitize("cfi")))
{
    ...
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(!inputDevice, invalidDistance,
        "CaptureSession::GetMinimumFocusDistance camera device is null");
    auto inputDeviceInfo = inputDevice->GetCameraDeviceInfo();
    CHECK_RETURN_RET_ELOG(!inputDeviceInfo, invalidDistance,
    ...
    CHECK_RETURN_RET_ELOG(metadata == nullptr, invalidDistance,
...
ColorSpaceInfo CaptureSession::GetSupportedColorSpaceInfo()
{
    ...
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, colorSpaceInfo,
...
std::vector<ColorEffect> CaptureSession::GetSupportedColorEffects()
{
    ...
    std::shared_ptr<Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, supportedColorEffects,
...
int32_t CaptureSession::GetSensorExposureTimeRange(std::vector<uint32_t> &sensorExposureTimeRange)
{
    ...
    std::shared_ptr<OHOS::Camera::CameraMetadata> metadata = GetMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, CameraErrorCode::INVALID_ARGUMENT,
...
int32_t CaptureSession::EnableMacro(bool isEnable)
{
    ...
    CHECK_RETURN_RET_ELOG(!IsMacroSupported(), CameraErrorCode::OPERATION_NOT_ALLOWED,
...
bool CaptureSession::IsLowLightBoostSupported()
{
    ...
    for (uint32_t i = 0; i < item.count - 1; i += step) {
        CHECK_RETURN_RET(GetMode() == item.data.i32[i] && item.data.i32[i + 1] == 1, true);
    }
    return false;
}
...
int32_t CaptureSession::EnableAutoCloudImageEnhancement(bool enabled)
{
    ...
    UnlockForControl();
    CHECK_PRINT_ELOG(res != CameraErrorCode::SUCCESS, "CaptureSession::EnableAutoCloudImageEnhancement Failed");
    return res;
}
...
std::vector<PortraitEffect> CaptureSession::GetSupportedPortraitEffects()
{
    ...
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(!inputDevice, supportedPortraitEffects,
...
int32_t CaptureSession::EnableAutoDeviceSwitch(bool isEnable)
{
    ...
    // LCOV_EXCL_START
    CHECK_RETURN_RET_ELOG(GetIsAutoSwitchDeviceStatus() == isEnable, CameraErrorCode::SUCCESS,
        "Repeat Settings.");
...
bool CaptureSession::SwitchDevice()
{
    ...
    CHECK_RETURN_RET_ELOG(!IsSessionStarted(), false,
    ...
    CHECK_RETURN_RET_ELOG(retCode != CameraErrorCode::SUCCESS, false,
