...
int32_t CaptureSession::AddOutput(sptr<CaptureOutput>& output, bool isVerifyOutput)
{
    ...
    if (!IsSessionConfiged()) {
        ...
        return InnerErrorCode::OPERATION_NOT_ALLOWED_OF_SESSION_READY;
    }
    if (output == nullptr) {
    ...
    CHECK_RETURN_RET_ELOG(ConfigureOutput(output) != CameraErrorCode::SUCCESS,
        CheckSAErrorCode(InnerErrorCode::SERVICE_FATL_ERROR_OF_CONFIG),
    ...
    CHECK_RETURN_RET_ELOG(isVerifyOutput && !CanAddOutput(output),
        CheckSAErrorCode(InnerErrorCode::SERVICE_FATL_ERROR_OF_CONFIG),
...
int32_t CaptureSession::AddSecureOutput(sptr<CaptureOutput> &output)
{
    ...
    if (repeatStream == nullptr) {
        MEDIA_ERR_LOG("CaptureSession::AddSecureOutput repeatStream is nullptr");
...
int32_t CaptureSession::SetVideoStabilizationMode(VideoStabilizationMode stabilizationMode)
{
    ...
    CHECK_RETURN_RET(!IsVideoStabilizationModeSupported(stabilizationMode),
                     CheckSAErrorCode(InnerErrorCode::OPERATION_NOT_ALLOWED_OF_UNSUPPORTED_FEATURE));
...
int32_t CaptureSession::SetExposureBias(float exposureValue)
{
    ...
    auto inputDevice = GetInputDevice();
    CHECK_RETURN_RET_ELOG(!inputDevice, CheckSAErrorCode(InnerErrorCode::OPERATION_NOT_ALLOWED_OF_DEVICE),
                          "CaptureSession::SetExposureBias camera device is null");
    auto inputDeviceInfo = inputDevice->GetCameraDeviceInfo();
    CHECK_RETURN_RET_ELOG(!inputDeviceInfo, CheckSAErrorCode(InnerErrorCode::OPERATION_NOT_ALLOWED_OF_DEVICE),
...
int32_t CaptureSession::SetFocusMode(FocusMode focusMode)
{
    ...
    CHECK_RETURN_RET(!IsFocusModeSupported(focusMode),
        CheckSAErrorCode(InnerErrorCode::OPERATION_NOT_ALLOWED_OF_UNSUPPORTED_FEATURE));
...
int32_t CaptureSession::SetFlashMode(FlashMode flashMode)
{
    ...
    CHECK_RETURN_RET(!IsFlashModeSupported(flashMode),
        CheckSAErrorCode(InnerErrorCode::OPERATION_NOT_ALLOWED_OF_UNSUPPORTED_FEATURE));
...
int32_t CaptureSession::EnableMacro(bool isEnable)
{
    ...
    CHECK_RETURN_RET_ELOG(!IsMacroSupported(),
                          CheckSAErrorCode(InnerErrorCode::OPERATION_NOT_ALLOWED_OF_UNSUPPORTED_FEATURE),
...
int32_t CaptureSession::ValidateSessionForZoom()
{
    CHECK_RETURN_RET_ELOG(!IsSessionCommited(), CameraErrorCode::SESSION_NOT_CONFIG,
...
int32_t CaptureSession::GetZoomPointMetadata(const std::shared_ptr<Camera::CameraMetadata>& metadata,
                                             camera_metadata_item_t& item)
{
...
bool CaptureSession::ValidateSceneModeLength(uint32_t currentIndex, uint32_t totalCount, int32_t length)
{
    ...
    if (length < minLen || length > maxLen) {
        MEDIA_ERR_LOG("CaptureSession::GetZoomPointInfos invalid len:%{public}d, must be 0-4", length);
        return false;
    }
    if (currentIndex + length * skipLen > totalCount) {
        MEDIA_ERR_LOG("CaptureSession::GetZoomPointInfos insufficient data, need:%{public}u, have:%{public}u",
            currentIndex + length * skipLen, totalCount);
        return false;
    }
    return true;
}
                                          ...
                                          std::vector<ZoomPointInfo>& zoomPointInfoList)
{
    ...
    for (uint32_t i = 0; i < item.count;) {
        ...
        if (!ValidateSceneModeLength(i, item.count, len)) {
            break;
        }

        if (sceneMode == currentMode) {
            for (int32_t j = 0; j < len; j++) {
                ZoomPointInfo zoomPointInfo = ParseZoomPointInfo(item, i);
...
int32_t CaptureSession::GetZoomPointInfos(std::vector<ZoomPointInfo>& zoomPointInfoList)
{
    ...
    CHECK_RETURN_RET_ELOG(inputDevice == nullptr, CameraErrorCode::SESSION_NOT_CONFIG,
        "CaptureSession::GetZoomPointInfos input device is null");

    auto inputDeviceInfo = inputDevice->GetCameraDeviceInfo();
    CHECK_RETURN_RET_ELOG(inputDeviceInfo == nullptr, CameraErrorCode::SESSION_NOT_CONFIG,
        "CaptureSession::GetZoomPointInfos input device info is null");

    std::shared_ptr<Camera::CameraMetadata> metadata = inputDeviceInfo->GetCachedMetadata();
    CHECK_RETURN_RET_ELOG(metadata == nullptr, CameraErrorCode::SESSION_NOT_CONFIG,
        "CaptureSession::GetZoomPointInfos metadata is null");

    int32_t retCode = ValidateSessionForZoom();
    CHECK_RETURN_RET_ELOG(retCode != CameraErrorCode::SUCCESS, retCode,
        "CaptureSession::GetZoomPointInfos validate session failed, retCode:%{public}d", retCode);

    camera_metadata_item_t item;
    retCode = GetZoomPointMetadata(metadata, item);
    CHECK_RETURN_RET_ELOG(retCode != CameraErrorCode::SUCCESS, CameraErrorCode::SUCCESS,
        "CaptureSession::GetZoomPointInfos get zoom point metadata failed, retCode:%{public}d", retCode);

    SceneMode currentMode = GetMode();
    MEDIA_DEBUG_LOG("CaptureSession::GetZoomPointInfos current mode:%{public}d", currentMode);

    // HDI data format: mode,len(0-4),zoom1,equivalent1,zoom2,equivalent2...
    // Traverse data, match current mode
    ProcessSceneModeData(item, currentMode, zoomPointInfoList);
    MEDIA_DEBUG_LOG("CaptureSession::GetZoomPointInfos found %{public}zu zoom points for mode:%{public}d",
