...
// SensorExposureTime
int32_t ProfessionSession::GetSensorExposureTimeRange(std::vector<uint32_t> &sensorExposureTimeRange)
{
    sensorExposureTimeRange.clear();
    if (!IsSessionCommited()) {
        MEDIA_ERR_LOG("ProfessionSession::GetSensorExposureTimeRange Session is not Commited");
        return CameraErrorCode::SESSION_NOT_CONFIG;
    }
    if (!inputDevice_ || !inputDevice_->GetCameraDeviceInfo()) {
        MEDIA_ERR_LOG("ProfessionSession::GetSensorExposureTimeRange camera device is null");
        return CameraErrorCode::INVALID_ARGUMENT;
    }
    std::shared_ptr<OHOS::Camera::CameraMetadata> metadata = GetMetadata();
    camera_metadata_item_t item;
    ...
    if (ret != CAM_META_SUCCESS || item.count == 0) {
        MEDIA_ERR_LOG("ProfessionSession::GetSensorExposureTimeRange Failed with return code %{public}d", ret);
        return CameraErrorCode::INVALID_ARGUMENT;
    }
...
int32_t ProfessionSession::SetSensorExposureTime(uint32_t exposureTime)
{
    if (!IsSessionCommited()) {
        MEDIA_ERR_LOG("ProfessionSession::SetSensorExposureTime Session is not Commited");
        return CameraErrorCode::SESSION_NOT_CONFIG;
    }
    if (changedMetadata_ == nullptr) {
        MEDIA_ERR_LOG("ProfessionSession::SetSensorExposureTime Need to call LockForControl() "
            "before setting camera properties");
        return CameraErrorCode::SUCCESS;
    }
    ...
    if ((GetSensorExposureTimeRange(sensorExposureTimeRange) != CameraErrorCode::SUCCESS) &&
        ...
        MEDIA_ERR_LOG("ProfessionSession::SetSensorExposureTime range is empty");
        return CameraErrorCode::OPERATION_NOT_ALLOWED;
    }
    ...
    if (exposureTime != autoLongExposure && exposureTime < sensorExposureTimeRange[minIndex]) {
        MEDIA_DEBUG_LOG("ProfessionSession::SetSensorExposureTime exposureTime:"
...
int32_t ProfessionSession::GetSensorExposureTime(uint32_t &exposureTime)
{
    if (!IsSessionCommited()) {
        MEDIA_ERR_LOG("ProfessionSession::GetSensorExposureTime Session is not Commited");
        return CameraErrorCode::SESSION_NOT_CONFIG;
    }
    if (!inputDevice_ || !inputDevice_->GetCameraDeviceInfo()) {
        MEDIA_ERR_LOG("ProfessionSession::GetSensorExposureTime camera device is null");
        return CameraErrorCode::INVALID_ARGUMENT;
    }
    std::shared_ptr<OHOS::Camera::CameraMetadata> metadata = inputDevice_->GetCameraDeviceInfo()->GetMetadata();
    camera_metadata_item_t item;
    ...
    if (ret != CAM_META_SUCCESS) {
        MEDIA_ERR_LOG("ProfessionSession::GetSensorExposureTime Failed with return code %{public}d", ret);
        return CameraErrorCode::INVALID_ARGUMENT;
    }
    exposureTime = item.data.ui32[0];
    MEDIA_DEBUG_LOG("ProfessionSession::GetSensorExposureTime exposureTime: %{public}d", exposureTime);
    return CameraErrorCode::SUCCESS;
}
