...
void AuxiliaryPhotoListener::OnBufferAvailable()
{
    ...
    if (photoOutput_->GetDefaultTaskManager()) {
        wptr<AuxiliaryPhotoListener> thisPtr(this);
        photoOutput_->GetDefaultTaskManager()->SubmitTask([thisPtr]() {
...
void PhotoListenerAni::AssembleAuxiliaryPhoto(int64_t timestamp, int32_t captureId) __attribute__((no_sanitize("cfi")))
{
    ...
    if (photoOutput) {
        ...
        sptr<SurfaceBuffer> captureIdDepthMap;
        photoOutput->captureIdDepthMap_.Find(captureId, captureIdDepthMap);
        if (captureIdDepthMap && picture) {
            LoggingSurfaceBufferInfo(captureIdDepthMap, "deepSurfaceBuffer");
            picture->SetAuxiliaryPicture(captureIdDepthMap,
                OHOS::CameraStandard::CameraAuxiliaryPictureType::DEPTH_MAP);
            captureIdDepthMap = nullptr;
        }
...
void PhotoOutputImpl::CreateMultiChannelPictureLisenter(ani_env* env)
{
    if (pictureListener_ == nullptr) {
        ...
        if (photoOutput_->GetDefaultTaskManager() == nullptr) {
            constexpr int32_t auxiliaryPictureCount = 4;
            photoOutput_->SetDefaultTaskManager(
    ...
    std::shared_ptr<uintptr_t> callback)
{
    ...
    if (photoOutput_) {
        if (photoOutput_->GetDefaultTaskManager()) {
            photoOutput_->GetDefaultTaskManager()->CancelAllTasks();
            photoOutput_->GetDefaultTaskManager().reset();
            photoOutput_->SetDefaultTaskManager(nullptr);
        }
    }
}
