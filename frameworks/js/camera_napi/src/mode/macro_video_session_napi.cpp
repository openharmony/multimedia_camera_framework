...
napi_value MacroVideoSessionNapi::Init(napi_env env, napi_value exports)
{
    ...
    if (status == napi_ok) {
        ...
        if (status == napi_ok) {
            status = napi_set_named_property(env, exports, MACRO_VIDEO_SESSION_NAPI_CLASS_NAME, ctorObj);
...
napi_value MacroVideoSessionNapi::CreateCameraSession(napi_env env)
{
    ...
    if (status == napi_ok) {
        sCameraSession_ = CameraManager::GetInstance()->CreateCaptureSession(SceneMode::VIDEO_MACRO);
...
napi_value MacroVideoSessionNapi::MacroVideoSessionNapiConstructor(napi_env env, napi_callback_info info)
{
    ...
    if (status == napi_ok && thisVar != nullptr) {
        ...
        obj->macroVideoSession_ = static_cast<MacroVideoSession*>(sCameraSession_.GetRefPtr());
        obj->cameraSession_ = obj->macroVideoSession_;
        CHECK_ERROR_RETURN_RET_LOG(obj->macroVideoSession_ == nullptr, result, "macroVideoSession_ is null");
        status = napi_wrap(
