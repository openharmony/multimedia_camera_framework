...
napi_value CameraSessionNapi::CreateZoomPointInfoObject(napi_env env, const ZoomPointInfo& zoomPointInfo)
{
    napi_value value;
...
napi_value CameraSessionNapi::GetZoomPointInfos(napi_env env, napi_callback_info info)
{
    ...
    if (status == napi_ok && cameraSessionNapi != nullptr) {
        ...
        for (size_t i = 0; i < vecZoomPointInfoList.size(); i++) {
            napi_value value = CreateZoomPointInfoObject(env, vecZoomPointInfoList[i]);
            napi_set_element(env, result, i, value);
        }
    } else {
...
{
    MEDIA_INFO_LOG("EnableColorCube is called.");
    auto result = CameraNapiUtils::GetUndefinedValue(env);
    CameraSessionNapi* cameraSessionNapi = nullptr;
    napi_value argv[ARGS_ONE] = {0};
    CameraNapiParamParser jsParamParser(env, info, cameraSessionNapi, argv[PARAM0]);
    CHECK_RETURN_RET_ELOG(!jsParamParser.AssertStatus(PARAMETER_ERROR, "parse parameter occur error"), nullptr,
    ...
    if (cameraSessionNapi->cameraSession_ != nullptr) {
        int32_t retCode = cameraSessionNapi->cameraSession_->EnableColorCube(lutData);
        if (!CameraNapiUtils::CheckError(env, retCode)) {
            return result;
        }
    } else {
