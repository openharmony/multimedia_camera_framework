...
napi_value GetPhotoNapi(napi_env env, std::shared_ptr<OHOS::Media::NativeImage> nativeImage, bool isRaw)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(image == nullptr, nullptr, "%{public}s ImageNapi Create failed", __func__);
    if (!isRaw) {
...
{
    MEDIA_INFO_LOG("%{public}s Called", __func__);
    CHECK_ERROR_RETURN_RET_LOG(photoNapiPtr == nullptr, false, "%{public}s photoNapiPtr is nullptr", __func__);
