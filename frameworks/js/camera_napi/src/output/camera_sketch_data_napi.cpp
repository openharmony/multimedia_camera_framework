...
napi_value CameraSketchDataNapi::Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor cameraSketchDataProps[] = {
        DECLARE_NAPI_GETTER("ratio", GetCameraSketchRatio),
    ...
    status = napi_define_class(env, CAMERA_SKETCH_DATA_NAPI_CLASS_NAME, NAPI_AUTO_LENGTH,
                               CameraSketchDataNapiConstructor, nullptr,
                               sizeof(cameraSketchDataProps) / sizeof(cameraSketchDataProps[PARAM0]),
...
napi_value CameraSketchDataNapi::GetCameraSketchPixelMap(napi_env env, napi_callback_info info)
{
    ...
    if ((status == napi_ok) && (obj != nullptr) && (obj->sketchData_ != nullptr) &&
        ...
        napi_value jsResult = Media::PixelMapNapi::CreatePixelMap(obj->env_, obj->sketchData_->pixelMap);
        if (jsResult == nullptr) {
