...
void AsyncCompleteCallback(napi_env env, napi_status status, void* data)
{
    ...
    CHECK_ERROR_RETURN_LOG(videoOutputAsyncContext == nullptr,
...
napi_value VideoOutputNapi::CreateVideoOutputForTransfer(napi_env env, sptr<VideoOutput> videoOutput)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(videoOutput == nullptr, nullptr, "CreateVideoOutputForTransfer videoOutput is nullptr");
    napi_status status;
    napi_value result = nullptr;
    napi_value constructor;
...
{
    MEDIA_INFO_LOG("%{public}s Called", __func__);
    CHECK_ERROR_RETURN_RET_LOG(videoOutputNapiPtr == nullptr,
