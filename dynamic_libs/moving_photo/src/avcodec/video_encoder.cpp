...
int32_t VideoEncoder::Create(const std::string &codecMime)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(ret != AV_ERR_OK, 1, "Create AVCodecVideoEncoder failed, ret: %{public}d", ret);
    return 0;
}
...
int32_t VideoEncoder::Config()
{
    std::lock_guard<std::mutex> lock(encoderMutex_);
    CHECK_ERROR_RETURN_RET_LOG(avCodecProxy_ == nullptr, 1, "avCodecProxy_ is nullptr");
    CHECK_ERROR_RETURN_RET_LOG(!(avCodecProxy_->IsVideoEncoderExisted()), 1, "encoder is nullptr");
...
bool VideoEncoder::EnqueueBuffer(sptr<FrameRecord> frameRecord, int32_t keyFrameInterval)
{
    ...
    if (keyFrameInterval == KEY_FRAME_INTERVAL) {
        ...
        CHECK_ERROR_RETURN_RET_LOG(avCodecProxy_ == nullptr, false, "avCodecProxy_ is nullptr");
        int32_t ret = avCodecProxy_->AVCodecVideoEncoderSetParameter(format);
        CHECK_ERROR_RETURN_RET_LOG(ret != AV_ERR_OK, false, "SetParameter failed, ret: %{public}d", ret);
    }
...
bool VideoEncoder::EncodeSurfaceBuffer(sptr<FrameRecord> frameRecord)
{
    ...
    while (retryCount > 0) {
        ...
        CHECK_ERROR_RETURN_RET_LOG(!isStarted_ || avCodecProxy_ == nullptr || !avCodecProxy_->IsVideoEncoderExisted(),
