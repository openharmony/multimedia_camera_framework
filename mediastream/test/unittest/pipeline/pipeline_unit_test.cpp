...
void PiplineUnitTest::SetUp(void)
{
    ...
    testId = std::string("Test_") + std::to_string(Pipeline::GetNextPipelineId());
    pipeline_->Init(nullptr, nullptr, testId);
    filterOne_ = std::make_shared<TestFilter>("filterOne", CFilterType::AUDIO_CAPTURE);
    filterTwo_ = std::make_shared<TestFilter>("filterTwo", CFilterType::AUDIO_ENC);
}
...
/**
 * @tc.name: Pipeline_Test_AddHeadFilters_0101
 * @tc.desc: Pipeline_Test_AddHeadFilters_0101
 * @tc.type: FUNC
 */
HWTEST_F(PiplineUnitTest, Pipeline_Test_AddHeadFilters_0101, TestSize.Level1)
{
    EXPECT_EQ(pipeline_->AddHeadFilters({}), Status::OK);
}

/**
 * @tc.name: Pipeline_Test_AddHeadFilters_0103
 * @tc.desc: Pipeline_Test_AddHeadFilters_0103
 * @tc.type: FUNC
 */
HWTEST_F(PiplineUnitTest, Pipeline_Test_AddHeadFilters_0103, TestSize.Level1)
{
...
HWTEST_F(PiplineUnitTest, Pipeline_Test_LinkFilters_0100, TestSize.Level1)
{
    ...
    EXPECT_EQ(pipeline_->LinkFilters(filterOne_, {filterTwo_},
        CStreamType::ENCODED_AUDIO), Status::OK);
...
/**
 * @tc.name: Pipeline_Test_Preroll_0100
 * @tc.desc: Pipeline_Test_Preroll_0100
 * @tc.type: FUNC
 */
HWTEST_F(PiplineUnitTest, Pipeline_Test_Preroll_0100, TestSize.Level1)
{
    EXPECT_EQ(pipeline_->AddHeadFilters({filterOne_}), Status::OK);
    EXPECT_EQ(pipeline_->Preroll(true), Status::OK);
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(PiplineUnitTest, Pipeline_Test_Preroll_0101, TestSize.Level1)
{
    EXPECT_EQ(pipeline_->AddHeadFilters({filterOne_}), Status::OK);
    EXPECT_EQ(pipeline_->Preroll(false), Status::OK);
}
} // namespace CameraStandard
} // namespace OHOS
