...
private:
    std::shared_ptr<MetaDataFilter> metaDataFilter_ {nullptr};
};
...
public:
    ...
    inline void GetMuxerFirstFrameTimestamp(int64_t& timestamp)
    {
        timestamp = firstFrameTimestamp_;
    };
    inline void SetMuxerFirstFrameTimestamp(int64_t timestamp)
    {
        firstFrameTimestamp_ = timestamp;
    };

    MockNextFilter(std::string name = "MockMetaDataNextFilter", CFilterType type = CFilterType::MUXER)
        : CFilter(name, type)
    {
