...
#ifndef AUDIO_PROCESS_FILTER_UNIT_TEST_H
#define AUDIO_PROCESS_FILTER_UNIT_TEST_H
...
class AudioProcessFilterUnitTest : public testing::Test {
public:
...
private:
    std::shared_ptr<AudioProcessFilter> audioProcessFilter_ {nullptr};
};
...
public:
    ...
    MockPrevFilter() : CFilter("MockAudPrevFilter", CFilterType::AUDIO_CAPTURE)
    {
...
public:
    ...
    MockNextFilter(std::string name = "MockAudNextFilter", CFilterType type = CFilterType::AUDIO_ENC)
        : CFilter(name, type)
    {
        ON_CALL(*this, LinkNext(_, _)).WillByDefault(Return(Status::OK));
        ON_CALL(*this, OnLinked(_, _, _)).WillByDefault(Return(Status::OK));
