...
#ifndef AUDIO_FORK_FILTER_UNIT_TEST_H
#define AUDIO_FORK_FILTER_UNIT_TEST_H
...
class AudioForkFilterUnitTest : public testing::Test {
public:
...
private:
    std::shared_ptr<AudioForkFilter> audioForkFilter_ {nullptr};
};
...
public:
    ...
    MockPrevFilter() : CFilter("MockAudPrevFilter", CFilterType::AUDIO_ENC)
    {
...
public:
    ...
    MockNextFilter(std::string name = "MockAudNextFilter", CFilterType type = CFilterType::MUXER)
        : CFilter(name, type)
    {
        ON_CALL(*this, LinkNext(_, _)).WillByDefault(Return(Status::OK));
        ON_CALL(*this, OnLinked(_, _, _)).WillByDefault(Return(Status::OK));
