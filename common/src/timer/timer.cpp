...
{
    MEDIA_DEBUG_LOG("name: %s, timer type: %{public}d(0: once, 1: periodic), intervalMs: %u",
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    MEDIA_DEBUG_LOG("stop timer (%s), timer type: %{public}d(0: once, 1: periodic), intervalMs: %u",
        name_.c_str(), timerType_, intervalMs_);
    if (!active_) {
        return true;
    }
...
bool Timer::StartUnlocked(uint32_t delayTimeMs)
{
    ...
    MEDIA_DEBUG_LOG("timer (%s), type: %{public}d(0: once, 1: periodic), curTime: %{public}d, expiredTime: %{public}d",
...
{
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (!active_) {
            MEDIA_DEBUG_LOG("inactive timer (%s) expired, type: %{public}d(0: once, 1: periodic), intervalMs: %u",
                name_.c_str(), timerType_, intervalMs_);
            return;
        }
    ...
    MEDIA_DEBUG_LOG("timer (%s) expired, type: %{public}d(0: once, 1: periodic), expiredTimeMs at %{public}d",
