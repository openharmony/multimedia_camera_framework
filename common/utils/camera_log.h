...
#define POWERMGR_SYSEVENT_FOLD_STATE(bname, prestate, state, camid, campos)        \
    do {                                                                           \
        HiSysEventWrite(HiviewDFX::HiSysEvent::Domain::CAMERA, "FOLD_STATE",       \
                                     HiviewDFX::HiSysEvent::EventType::STATISTIC,  \
                                     "BUNDLE_NAME", bname, "PRE_STATE", prestate,  \
                                     "STATE", state, "CAMERA_ID", camid,           \
...
#define POWERMGR_SYSEVENT_CAMERA_CONFIG(type, width, height, camid, mode, nsmode,      \
    ...
    do {                                                                               \
        HiSysEventWrite(HiviewDFX::HiSysEvent::Domain::CAMERA, "CAMERA_CONFIG",        \
                                     HiviewDFX::HiSysEvent::EventType::STATISTIC,                                      \
                                     "TYPE", type, "WIDTH", width, "HEIGHT", height, "CAMERA_ID", camid, "MODE", mode, \
                                     "NIGHT_SUB_MODE", nsmode, "LIGHT_PAINTING_TYPE", lptype, "ZOOM_RATIO_RANGE", zoom,\
                                     "COUNT", count, "FORMAT", format, "FRAME_RATE_RANGE", fps, "COLOR_SPACE", cs,     \
                                     "IS_COLOR_SPACE_SETTED", iscs, "STABILIZATION_MODE", stabmode, "ERROR_CODE", err);\
    } while (0)
