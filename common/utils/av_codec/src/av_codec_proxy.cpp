    ...
    : avcodecLib_(avcodecLib), avcodecIntf_(avcodecIntf)
{
    ...
    CHECK_ERROR_RETURN_LOG(avcodecLib_ == nullptr, "avcodecLib is nullptr");
    CHECK_ERROR_RETURN_LOG(avcodecIntf_ == nullptr, "avcodecIntf is nullptr");
}

AVCodecProxy::~AVCodecProxy()
{
    MEDIA_DEBUG_LOG("AVCodecProxy destructor");
    CHECK_EXECUTE(avcodecIntf_ != nullptr, avcodecIntf_ = nullptr);
    CHECK_EXECUTE(avcodecLib_ != nullptr, avcodecLib_ = nullptr);
    MEDIA_DEBUG_LOG("AVCodecProxy destructor end");
}
...
std::shared_ptr<AVCodecProxy> AVCodecProxy::CreateAVCodecProxy()
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(dynamiclib == nullptr, nullptr, "Failed to load media library");
...
int32_t AVCodecProxy::CreateAVMuxer(int32_t fd, OutputFormat format)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(avcodecIntf_ == nullptr, AV_ERR_INVALID_VAL, "avcodecIntf_ is nullptr");
    auto ret = avcodecIntf_->CreateAVMuxer(fd, format);
    CHECK_ERROR_RETURN_RET_LOG(ret != AV_ERR_OK, AV_ERR_FAILED, "CreateAVMuxer failed, ret: %{public}d", ret);
...
int32_t AVCodecProxy::CreateAVCodecVideoEncoder(const std::string& codecMime)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(ret != AV_ERR_OK, AV_ERR_FAILED,
...
int32_t AVCodecProxy::AVMuxerSetParameter(const std::shared_ptr<Meta>& param)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(avcodecIntf_ == nullptr, AV_ERR_INVALID_VAL, "avcodecIntf_ is not created");
...
int32_t AVCodecProxy::AVMuxerWriteSample(uint32_t trackIndex, const std::shared_ptr<AVBuffer>& sample)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(ret != AV_ERR_OK, AV_ERR_FAILED, "AVMuxerWriteSample failed, ret: %{public}d", ret);
...
bool AVCodecProxy::IsVideoEncoderExisted()
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(avcodecIntf_ == nullptr, false, "avcodecIntf_ is not created");
...
sptr<Surface> AVCodecProxy::CreateInputSurface()
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(avcodecIntf_ == nullptr, nullptr, "avcodecIntf_ is not created");
...
void AVCodecProxy::CreateAVSource(int32_t fd, int64_t offset, int64_t size)
{
    ...
    CHECK_ERROR_RETURN_LOG(avcodecIntf_ == nullptr, "avcodecIntf_ is nullptr");
...
int32_t AVCodecProxy::ReadSampleBuffer(uint32_t trackIndex, std::shared_ptr<AVBuffer> sample)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(ret != AV_ERR_OK, AV_ERR_FAILED, "ReadSampleBuffer failed, ret: %{public}d", ret);
...
int32_t AVCodecProxy::AVDemuxerSeekToTime(int64_t millisecond, SeekMode mode)
{
    ...
    CHECK_ERROR_RETURN_RET_LOG(ret != AV_ERR_OK, AV_ERR_FAILED, "AVDemuxerSeekToTime failed, ret: %{public}d", ret);
