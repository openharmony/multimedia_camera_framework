...
#ifndef OHOS_CAMERA_RECORDER_ENGINE_PROXY_H
#define OHOS_CAMERA_RECORDER_ENGINE_PROXY_H

#include "recorder_engine_interface.h"
#include "camera_dynamic_loader.h"

namespace OHOS::CameraStandard {
class RecorderEngineProxy : public RecorderEngineIntf {
public:
    explicit RecorderEngineProxy(
        std::shared_ptr<Dynamiclib> mediaStreamLib, std::shared_ptr<RecorderEngineIntf> recorderEngineIntf);
    ~RecorderEngineProxy() override;
    static std::shared_ptr<RecorderEngineProxy> CreateRecorderEngineProxy();
    static void FreeMediaStreamDynamiclib();
...
private:
    ...
    std::shared_ptr<Dynamiclib> mediaStreamLib_{nullptr};
    std::shared_ptr<RecorderEngineIntf> recorderEngineIntf_{nullptr};
};
} // namespace OHOS::CameraStandard
#endif // OHOS_CAMERA_RECORDER_ENGINE_PROXY_H