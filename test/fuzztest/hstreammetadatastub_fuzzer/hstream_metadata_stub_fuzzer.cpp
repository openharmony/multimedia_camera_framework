...
static size_t g_pos;
static const std::u16string g_interfaceToken = u"OHOS.CameraStandard.IStreamMetadata";
...
void Test()
{
    ...
    dataMessageParcel.WriteInterfaceToken(g_interfaceToken);
    dataMessageParcel.WriteRemoteObject(proxy->AsObject());
    HStreamMetadataStubFuzzer::fuzz_->OnRemoteRequestInner(
        static_cast<uint32_t>(IStreamMetadataIpcCode::COMMAND_SET_CALLBACK), dataMessageParcel, reply, option);
}
