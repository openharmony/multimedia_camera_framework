...
void DeferredPhotoProcessingSessionStubFuzzer::OnRemoteRequest(int32_t code)
{
...
void DeferredPhotoProcessingSessionStubFuzzer::FuzzTest()
{
    if ((RAW_DATA == nullptr) || (g_dataSize > MAX_CODE_LEN) || (g_dataSize < MIN_SIZE_NUM)) {
        return;
    }
    ...
    manager_->DisableAutoSuspend(time);
}

void Test()
{
    auto deferredPhotoProcessingSessionStub = std::make_unique<DeferredPhotoProcessingSessionStubFuzzer>();
    if (deferredPhotoProcessingSessionStub == nullptr) {
        MEDIA_INFO_LOG("deferredPhotoProcessingSessionStub is null");
        return;
    }
    for (uint32_t i = 0; i <= MAX_CODE_NUM; i++) {
        deferredPhotoProcessingSessionStub->OnRemoteRequest(i);
    }
    deferredPhotoProcessingSessionStub->FuzzTest();
}
