...
void Test(uint8_t *rawData, size_t size)
{
    ...
    CHECK_RETURN_ELOG(!photoSurface, "StreamMetadataStubFuzzer: Create photoSurface Error");
...
void Test_OnRemoteRequest(uint8_t *rawData, size_t size)
{
    ...
    code  = static_cast<uint32_t>(IStreamMetadataIpcCode::COMMAND_START);
    data.RewindRead(0);
    fuzz_->OnRemoteRequest(code, data, reply, option);

    code = static_cast<uint32_t>(IStreamMetadataIpcCode::COMMAND_STOP);
    data.RewindRead(0);
    fuzz_->OnRemoteRequest(code, data, reply, option);

    code = static_cast<uint32_t>(IStreamMetadataIpcCode::COMMAND_RELEASE);
    data.RewindRead(0);
    fuzz_->OnRemoteRequest(code, data, reply, option);
